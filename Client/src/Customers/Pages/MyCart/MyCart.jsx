...
const MyCart = () => {
  ...
  const fetchCart = async () => {
    try {
      const res = await axios.get(`http://localhost:5000/bookingWithCart/${userId}`);
  ...
  const handleCheckout = async () => {
    await axios.put(`http://localhost:5000/BookingPutCheck/${bookingData._id}`, {
      bookAmount: totalAmount,
       bookStatus:1
    });

    navigate(`/customer/bookingconfirm/${bookingData._id}`);
  };
